            ...
            if (telegrambotsql.checkChatId(message.getChatId())) {
                switch (message.getText()) {
                    ...
                    case "\uD83D\uDCAFТекущие оценки":
                        ...
                        break;
                    case "Успеваемость":
                        pdfMaker.createUniverTranskriptPdf(telegrambotsql.getfromBotsName(message.getChatId()),"Транскрипт", Univer.getTranskript(telegrambotsql.getIIN(message.getChatId())), Univer.getGPAforTranskript(telegrambotsql.getIIN(message.getChatId())));
